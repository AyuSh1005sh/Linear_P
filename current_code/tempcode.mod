...
dvar boolean gDecided[I][J][edges];
...
subject to {   
    ...
    forall(i in I, n in nodes[i],j in J, k in edges: n != 1 && n != addr[j]){
    ...
    forall(i in I, n in nodes[i],j in J, k in edges){    
    ...
    forall(i in I, j in J, k in edges, tm in T) {
